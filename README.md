...
牌表弹窗和正文卡名链接使用 `decklist-modal.css` / `decklist-modal.js`。启用 `window.tcgDecklistConfig.articleCardLinks` 后，正文命中的牌表卡名会自动生成 `.tcg-card-link`：绿金点状下划线，点击打开卡图，悬停显示卡图预览。没有牌表弹窗的文章页可用 `window.tcgArticleCardLinkConfig` 单独启用同一组件。手写卡图链接也应复用 `.tcg-card-link`，并提供 `data-card-image`、`data-card-name` 和 `href`。
